...
const MOCK_DATA: Quotation[] = [
    { id: '#QTN-2026-501', description: 'Bulk Chemical Supply', client: 'Alice Smith', clientCompany: 'SKY SHORE PVT LTD', validUntil: 'Feb 15, 2026', total: 'USD 42,000.00', status: 'Approved', date: 'Jan 22, 2026' },
    { id: '#QTN-2026-502', description: 'Construction Steel', client: 'Bob Builder', clientCompany: 'BUILDERS CO', validUntil: 'Feb 10, 2026', total: 'USD 32,000.00', status: 'Pending', date: 'Jan 21, 2026' },
    { id: '#QTN-2026-503', description: 'Office IT Upgrade', client: 'Charlie Day', clientCompany: 'TECH ENTERPRISES', validUntil: 'Jan 30, 2026', total: 'USD 15,500.00', status: 'Draft', date: 'Jan 21, 2026' },
    { id: '#QTN-2026-504', description: 'Safety Equipment', client: 'Dan Brown', clientCompany: 'SAFE SECURE', validUntil: 'Feb 01, 2026', total: 'USD 8,200.00', status: 'Rejected', date: 'Jan 20, 2026' },
];

const groupData = (data: Quotation[]) => {
    const groups: { title: string; data: Quotation[] }[] = [];
    data.forEach(item => {
...
const getStatusColor = (status: string) => {
    switch (status) {
        case 'Approved': return { bg: '#10B981', text: '#FFFFFF' }; // Green
        case 'Pending': return { bg: '#F59E0B', text: '#FFFFFF' }; // Amber
        case 'Rejected': return { bg: '#EF4444', text: '#FFFFFF' }; // Red
        default: return { bg: '#64748B', text: '#FFFFFF' }; // Grey
    }
};

const QuotationsScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
            ...
            >
                ...
                <View style={styles.cardHeader}>
                    <View style={styles.headerLeft}>
                        ...
                        <View style={[styles.iconBox, { backgroundColor: theme.primaryLight + '20' }]}>
                            <MaterialCommunityIcons name="file-document-outline" size={18} color={theme.primaryLight} />
                        </View>

                        <View style={styles.headerTextContainer}>
                            <Text style={[styles.idText, { color: theme.primaryLight }]}>{item.id}</Text>
                            <Text style={styles.companyRow} numberOfLines={1}>
                                <Text style={[styles.companyName, { color: theme.text }]}>{item.clientCompany}</Text>
                ...
                <View style={[styles.detailsRow, { borderTopColor: theme.borderLight }]}>
                    <View style={styles.detailItem}>
                        ...
                        <Text style={[styles.detailValue, { color: theme.textSecondary }]}>{item.client}</Text>
                    </View>
                    <View style={styles.detailItem}>
                        <Text style={[styles.detailLabel, { color: theme.textTertiary }]}>Valid Until</Text>
                        <Text style={[styles.detailValue, { color: theme.textSecondary }]}>{item.validUntil}</Text>
                    </View>
                    <View style={styles.detailItem}>
                ...
                {!isSearchExpanded ? (
                    <>
                        <View style={styles.headerLeftContainer}>
                            ...
                            <Text style={[styles.headerTitle, { color: theme.headerText }]}>Quotations</Text>
                        </View>
                        ...
                        <View style={styles.headerRight}>
                            ...
                            <TouchableOpacity
                                onPress={() => navigation.navigate('GenericForm', { title: 'Quotation' })}
